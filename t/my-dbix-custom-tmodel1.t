...
ok($t5->{$tm1->name}->{is_deleted} eq "1", "sremove_by_id" . Dumper($t5));

$tm1->sremove($tt->{test_id}, 6);

my $t6 = $tm1->get_by_id($tt->{test_id});

ok($t6->{$tm1->name}->{is_deleted} eq "6", "sremove by id " . Dumper($t6));

$tm1->sremove($tt, 7);

my $t7 = $tm1->get_by_test_name($tt->{test_name});

ok($t7->{$tm1->name}->{is_deleted} eq "7", "sremove by obj " . Dumper($t7));

ok($t7->{$tm1->name} eq $t7->{list}->[0], "sremove by obj " . Dumper($t7));
